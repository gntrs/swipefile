#!/usr/bin/env bash
# Daily ads-stats pass. Runs on the WSL crontab (same box as
...
#    analytics from PostHog, everything from the database into export.json.
...
#
# Needs META_ACCESS_TOKEN + META_AD_ACCOUNT_ID in the WSL clone's .env
# (plus the usual DB_SERVICE_KEY and POSTHOG_API_KEY).
set -euo pipefail
...
# Daily KPI snapshot (funnel + traffic) into the database for the dashboard graphs.
...
node scripts/seo-rank-pull.mjs >> "$LOG" 2>&1 || echo "seo rank pull failed (continuing)" >> "$LOG"
node scripts/trends-pull.mjs >> "$LOG" 2>&1 || echo "trends pull failed (continuing)" >> "$LOG"
...
# --- 2. Analysis pass. ---
ANALYSIS_PROMPT='You are Claude running the automated daily ads-stats pass for this marketing dashboard. Fresh data was pulled seconds ago for you: .claude-data/export.json (all ads, posts, goals, briefs - our own ads are stored under the brand in env OWN_BRAND, their metrics straight from the Meta API) and .claude-data/posthog.json (site traffic + funnel events).
